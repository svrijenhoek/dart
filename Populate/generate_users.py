...
for x in range(0, n_users):
    ...
    for y in range(0, n_topics):
        document = connector.get_random_document('articles')[0]
        spread = max(Util.get_random_number(n_spread, n_spread/2), 5)
        response = connector.get_similar_documents('articles', document['_source']['id'], spread)
    ...
    response = connector.get_most_popular(n_popular)
    for hit in response:
    ...
    for y in range(0, n_random):
        article = connector.get_random_document('articles')[0]
